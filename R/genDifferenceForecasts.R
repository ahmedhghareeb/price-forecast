# Name: Generate difference model forecasts
# 
# Description: Script generates forecasts using daily weather data files and
# difference model produced by testDiffModel.R
# 
...
#### Load data ================================================================
load("./cache/DifferenceModel.RData")
...
pricesLagged = pricesLastWeek %>% 
  ...
  mutate(ts = ts + days(7),
         ...
         Price_d168_l168 = lag(Price_d168)) %>% 
  na.omit()
rm(pricesLastWeek)
...
for (i in 0:23) {
  ...
  price_tmp <- weather %>% 
    filter(Hour==i) %>% 
    mutate(predictions = predictions + Price_l168)
