...
subDate <- ymd("2016-04-17", tz="UTC")
n_data <- "All"

#### Run model with latest data ===============================================
source("./R/testModels_HourModels_adjHrs_1day.R")
...
pricesLagged = pricesLastWeek %>% 
  ...
  mutate(ts = ts + days(7)) %>% 
  mutate(Price_l24 = lead(Price_l168, 144),
         Price_l1 = lead(Price_l168, 167),
         Price_l2 = lead(Price_l168, 166),
         Price_l3 = lead(Price_l168, 165))
rm(pricesLastWeek)
...
weather <- inner_join(weather, pricesLagged) %>% 
  filter(!is.na(Price_l24)) #stops errors in prediction
...
write.csv(outputForecasts, file = paste0("./cache/", filename, ".csv"),
          row.names=F, quote=F)
save(outputForecasts, file = paste0("./cache/1_day_", filename, ".RData"))
